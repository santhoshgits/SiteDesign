...
if len(sys.argv) == 6:
	...
	print ('PocketApprox.py <SiteBinaryFolder> <Pairs.txt> <OutFile.txt> <No.of.Cores> <Fold>') # site1.pdb site2.pdb
	sys.exit()
...
def BatchRun(PdbPairs):
	...
	if os.path.exists(dire+'/'+BinFold+'/'+site1) and os.path.exists(dire+'/'+BinFold+'/'+site2):
		...
		if score < 0:
			score = 0
		score = float(score)	
		minim = sorted( [round(score/L1, 2), round(score/L2, 2)] )
...
aline = open(dire+'/'+files,'r').readlines()
