...
class BuildStretch():
	...
	def file_read(self,  nos, ligand_aline, res_taken, rng):
		global NoMapReslen 
		random.shuffle(res_taken)
		for no in range(nos):
			prev, cur = 0, 0
...
'''
res_point_line = open(dire+'/mf8/residue_position', 'r').readlines()	
ligand_aline = open(dire+'/mf8/lig.pdb', 'r').readlines()	
site_gen = BuildStretch(res_point_line, ligand_aline, 'mf8')

#site_gen.file_read(1, ligand_aline, 19, 27)	
...
dire = os.getcwd()
aline = open(dire+'/mf8/final2.pdb', 'r').readlines()
for line in aline:
	line = line.strip()
...
for i in range(nos):	
	t = time.time()
	sites = site_gen.file_read(1, ligand_aline, 19, 27)	
	#sites = site_gen.file_read(1, ligand_aline, 10, 15)	
	out = open(dire+'/temp2a/'+str(i)+'a.pdb', 'w')
